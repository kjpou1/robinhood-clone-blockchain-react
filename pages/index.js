...
export const getStaticProps = async () => {
  ...
  const response = await fetch('https://coinranking1.p.rapidapi.com/coins?referenceCurrencyUuid=yhjMzLPhuIDl&timePeriod=24h&tiers=1&orderBy=marketCap&orderDirection=desc&limit=50&offset=0', options)
  const apiResponse = await response.json();
  let coins = []
  if (apiResponse.status === 'success')
  {
    coins = apiResponse.data.coins
  }
  return {
    props: { coins },
  }
